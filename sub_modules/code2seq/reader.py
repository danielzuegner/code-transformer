    ...
    def __init__(self, subtoken_to_index, target_to_index, node_to_index, config, is_evaluating=False,
        ...
        if self.use_multilanguage:
            ...
            for lang in self.languages:
                ...
                self.file_path[lang] = str(Path(config.TEST_PATH).with_name(lang)) + '.' + eval_dataset + '.c2s' \
                    if is_evaluating \
    ...
    def compute_output(self):
        if self.use_multilanguage:
            ...
            for lang, file_path in self.file_path.items():
                single_dataset = tf.data.experimental.CsvDataset(file_path, record_defaults=self.record_defaults,
                                                                 field_delim=' ',
            ...
            dataset = tf.data.experimental.CsvDataset(self.file_path, record_defaults=self.record_defaults,
                                                      field_delim=' ',
